...
from database.db import _db, _new_id, _now
import logging

logger = logging.getLogger(__name__)

def add_student(roll_no: str, name: str, email: str = "", phone: str = "", department: str = "", semester: int = 0, gpa: float = 0.0) -> str:
    student_id = _new_id()
    with _db() as conn:
        conn.execute(
        ...
        for s in students_list:
            try:
                conn.execute(
                    "INSERT INTO students (id, roll_no, name, email, phone, department, semester, gpa, uploaded_at) "
                    "VALUES (?,?,?,?,?,?,?,?,?)",
                    (_new_id(), s.get('roll_no'), s.get('name'), s.get('email', ''), s.get('phone', ''),
                     s.get('department', ''), s.get('semester', 0), s.get('gpa', 0.0), _now()),
                )
                count += 1
            ...
            try:
                ...
                if student_id:
                    ...
                    conn.execute(
                        ...
                        "VALUES (?,?,?,?,?,?,?,?,?,?)",
                        (_new_id(), student_id, m.get('subject_code'), m.get('subject_name'), m.get('semester', 1),
                         internal, external, internal + external, m.get('grade', ''), _now()),
                    )
                    count += 1
            except Exception as e:
...
def query_student_marks(roll_no: str = "", subject_code: str = "", semester: int = 0) -> list[dict]:
    with _db() as conn:
        if roll_no:
            ...
            if subject_code:
                rows = conn.execute("SELECT * FROM student_marks WHERE student_id = ? AND subject_code = ?", (student_id, subject_code)).fetchall()
            elif semester:
                rows = conn.execute("SELECT * FROM student_marks WHERE student_id = ? AND semester = ?", (student_id, semester)).fetchall()
            else:
                rows = conn.execute("SELECT * FROM student_marks WHERE student_id = ?", (student_id,)).fetchall()
        else:
            return []
