...
**Signal Groups:**
```python
_OFFICIAL_SITE = keywords: ["college", "admission", "fees", "placement", ...]
_DOCUMENT = keywords: ["policy", "handbook", "syllabus", "regulation", ...]
_TIMETABLE = keywords: ["timetable", "schedule", "classroom", "period", ...]
_FAQ = keywords: ["what is", "how to", "when is", "eligibility", ...]
_GENERAL_CHAT = keywords: ["hello", "hi", "thanks", ...]
```
...
### 4. Query Expansion (`rag/query_expansion.py`)

**Purpose:** Generates semantically diverse query variants and merges results with Reciprocal Rank Fusion. Activates automatically when retrieval score is low.

**Score-Based Triggering:**
- Query expansion activates **only when the initial retrieval score is below a threshold**
- Similar to HyDE: if `top_score < QUERY_EXPANSION_TRIGGER_SCORE`, expansion is triggered
- If score is good enough, expansion is skipped (no wasted LLM calls)
- Default threshold: 0.50

**Strategy:**
1. Retrieve with original query
2. Check top score
3. If score < threshold and expansion enabled, generate N rewrite variants
4. Retrieve candidates for each variant + original
5. Merge with Reciprocal Rank Fusion (RRF) so no single variant dominates
...
candidates = expand_and_retrieve(
    ...
    top_score=0.35,  # Pass top score from original retrieval
)
# If top_score < QUERY_EXPANSION_TRIGGER_SCORE, expands and returns RRF-merged list
# Otherwise skips expansion and returns original results
```

**Configuration (tests/config.py):**
```python
QUERY_EXPANSION_ENABLED = True                   # Enable/disable feature
QUERY_EXPANSION_TRIGGER_SCORE = 0.50             # Activate if top_score < this
...
**Example Flow:**
```
Original Query: "What are fees?"
→ Retrieve with original query
→ Top score: 0.35 (low)
→ Score 0.35 < threshold 0.50? YES
→ Generate variants: "What is the tuition?", "Cost of admission?", etc.
→ Retrieve for each variant
→ Merge with RRF
→ Return combined, higher-quality results
```

---

### 5. Conversation History (`rag/conversation_history.py`)
...
### 6. Provider Manager (`rag/providers/manager.py`)
...
### 4. Query Expansion (Score-Based)
Generates diverse paraphrases **only when retrieval score is low**, retrieves for each, merges with RRF. Improves recall on paraphrased questions without wasting LLM calls on good scores.
...
QUERY_EXPANSION_ENABLED = True              # Enable/disable feature
QUERY_EXPANSION_TRIGGER_SCORE = 0.50        # Activate if top_score < this
QUERY_EXPANSION_N = 3                       # Variants to generate
QUERY_EXPANSION_MAX_TOKENS = 150
QUERY_EXPANSION_RRF_K = 60                  # RRF constant
QUERY_EXPANSION_TEMPERATURE = 0.3
...
6. **Query Expansion Score-Based Triggering** *(NEW - May 2026)*
   - Query expansion now activates **only when retrieval score is low**
   - Similar to HyDE: if `top_score < QUERY_EXPANSION_TRIGGER_SCORE`, expansion triggered
   - Default threshold: 0.50 (configurable)
   - Avoids wasting LLM calls when score is already good
   - New config parameter: `QUERY_EXPANSION_TRIGGER_SCORE`
   - Improves efficiency while maintaining recall on difficult queries
...
- [QUERY_TO_VECTOR_SEARCH.md](QUERY_TO_VECTOR_SEARCH.md) — Detailed embedding and vector search flow
- [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md) — Conversation memory and query logs schema
