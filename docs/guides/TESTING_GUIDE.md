...
- Optional: `NVIDIA_TEMPERATURE` (default: `0.2`)
- If embeddings are not cached locally, allow model download once:
  - PowerShell: `setx HF_HUB_OFFLINE 0`

### Run (PowerShell)
```powershell
# Set NVIDIA API key for the evaluator
$env:NVIDIA_API_KEY = "your_nvidia_key"

# Hybrid retrieval run
python tests/ragas_eval.py --run-name hybrid --retrieval-mode hybrid --disable-memory

# Dense retrieval run
python tests/ragas_eval.py --run-name dense --retrieval-mode dense --disable-memory
```

### Output Files
The script writes results to `tests/ragas_outputs/`:
- `ragas_<run>_<timestamp>_per_sample.csv`
- `ragas_<run>_<timestamp>_summary.json`
- `ragas_<run>_<timestamp>_dataset.json` (questions, answers, contexts)

### Optional Overrides
- Limit rows: `--max-rows 10`
- Change top-k: `--top-k 5`
