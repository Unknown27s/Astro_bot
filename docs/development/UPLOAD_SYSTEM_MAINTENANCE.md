...
```
This script validates parsing, chunking, ChromaDB storage, and database writes end-to-end.
...
If upload errors appear again, check:
- Spring Boot logs (proxy layer)
- FastAPI logs (upload handler)
- `debug_upload.py` script output
